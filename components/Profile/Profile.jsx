...
const Profile = ({ info, client, wallet, setIsExist, merchants }) => {
    ...
    const [merchantCreator, setMerchantCreator] = useState(null);
    ...
    const loadBalance = async () => {
        const balance = await client.CosmosBankV1Beta1.query.queryAllBalances(
            wallet.address
        );
    ...
    const shareDataWith = async (merchant, attribute) => {
        try {
            const {
                [0]: { address },
            ...
            const payload = {
                ...
                creator: address,
                entity: merchantCreator,
            };
            console.log("payload", payload);
            const data = await client.BeltestEav.tx.sendMsgShareWith({
                value: payload,
            });
            console.log(data);
        } catch (error) {
